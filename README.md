...
- For recurring-task syncs, overwrite mode updates a routine in place only when the package contains exactly one recurring task with the same normalized title as exactly one active Paperclip routine. Description, status, and supported trigger changes are then applied to that routine. Package slug and file-path changes do not establish identity. If a recurring task title is renamed, the old routine is not renamed or archived automatically: the sync imports a new routine and leaves the unmatched old routine unchanged. Operators should archive the old routine explicitly, or keep the title stable when only changing cadence or other routine settings. If an otherwise matching update fails, sync falls back to the import-and-archive cleanup path.
