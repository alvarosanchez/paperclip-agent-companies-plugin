...
- A replace-mode sync whose saved task selection covers all tasks treats the verified routine UUID ledger as authoritative: a previously bound recurring task that is no longer in the source is archived by UUID after the import succeeds. Verified rename targets are protected, retries skip routines that are already archived, and partial task selections never infer deletion from an omitted task.
