...
## How This Relates To Built-In Paperclip Features

Paperclip 2026.831.1 already ships two things that overlap with this plugin, and it is worth knowing which tool to reach for.
...
- **Repository scanning.** Point it at `owner/repo`, a full git URL, or a local checkout and it discovers every `COMPANY.md` package in the repository, including multi-company repositories. Private repositories work through the host machine's existing git credential helpers; the built-in import handles a single public URL.
- **Partial selection.** Import or sync a chosen subset of agents, projects, tasks, recurring tasks, Paperclip issue manifests, and skills, with required dependencies auto-included. The built-in import applies the whole package.
- **Adapter presets.** Reusable named adapter configurations applied as an import default or a per-agent override, saved with the tracked import.
- **Tracked, revision-aware sync.** Each import saves a selection contract, the source version and git revision, and a verified source-to-Paperclip UUID ledger, which is what makes repeatable manual and hourly auto-sync, rename reconciliation, and replace-mode routine archival possible. Paperclip's own import is one-shot: it persists no source revision and offers no re-sync.

### What the host portability manifest does not carry
